...
contract TokenMaster is ReentrancyGuard, Pausable, Ownable{
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 tokenPerBlock; //token per block minted
     ...
     * _tokenPerBlock: the number of token that you want to mint per block in this specific farm
     ...
     * NOTE: mint is actually the wrong word for this case, because all the token are pre-minted and will be just transfered when people claim their rewards
     */
    function createPool(address _lpToken, string memory _symbol, uint16 _fee, address _vault, uint256 _tokenPerBlock, bool isVaultContract) external onlyOwner{
        require(tokenAlreadyInPool[_lpToken] == false);
        poolInfo.push(PoolInfo(IERC20(_lpToken), _symbol, _fee, _vault, isVaultContract, 0, _tokenPerBlock ,block.number, 0));
        if(isVaultContract){
    ...
    /**
     * @dev changes the token rewards per block
     * _pid: is the id of the pool that you want to apply this changes
     * _tokenPerBlock: is the new reward per block
     */
    function changePoolTokenReward(uint256 _pid, uint256 _tokenPerBlock) external onlyOwner{
        PoolInfo storage pool = poolInfo[_pid];
        pool.tokenPerBlock = _tokenPerBlock;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant{
        ...
        updatePool(_pid);
        token.safeTransfer(msg.sender, getPending(_pid, msg.sender));
    ...
    function updatePool(uint256 _pid) public{
        ...
        if(!pausedUpdatePools && !pausedPool[_pid]){
            ...
            if(lastBlock > block.number){
                rewardToDeliver = pool.tokenPerBlock.mul(block.number.sub(pool.lastRewardBlock));
            }else{
                rewardToDeliver = pool.tokenPerBlock.mul(lastBlock.sub(pool.lastRewardBlock));
            }
    ...
    function claim(uint256 _pid) external nonReentrant{
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        PoolInfo memory pool = poolInfo[_pid];
        if(token.transfer(msg.sender, getPending(_pid,msg.sender))){
          user.rewardMinus = user.amount.mul(pool.rewardsPerShare).div(1e18);
